...
public sealed class RecupInfosAnnoncesUseCase : IRecupInfosAnnoncesUseCase
{
	...
	static RecupInfosAnnoncesUseCase()
	{
	...
	public RecupInfosAnnoncesUseCase(IApplicationDbContext context, IExchangeService exchangeService)
	{
		_context = context;
		_exchangeService = exchangeService;
	}

	public async Task<AnnoncesGlobalesController.InfoAnnonces> Execute(AnnoncesGlobalesController.FilterDef filterDef)
	{
		...
		try
		{
			...
			List<AnnoncesGlobalesController.InfoAnnoncesRes> res = new List<AnnoncesGlobalesController.InfoAnnoncesRes>();
			for (int i = 0; i < filterDef.Fields.Length; i++)
			{
				if (RecupState.IsCanceled(state))
				{
					return new AnnoncesGlobalesController.InfoAnnonces
					{
				...
				for (int j = 0; j < filterDef.Fields.Length; j++)
				{
					...
					AnnoncesGlobalesController.FilterDefFieldInfo f = filterDef.Fields[j];
				...
				using (DbCommand cmd = c.CreateCommand())
				{
					...
					res.Add(new AnnoncesGlobalesController.InfoAnnoncesRes
					{
				...
				AnnoncesGlobalesController.FilterDefFieldInfo f2 = filterDef.Fields[i];
	...
	private static string GetDefaultWhere(AnnoncesGlobalesController.FilterDef filterDef)
	{
	...
	public static string ConvertFilterDefToAnnoncesGlobalesSQL(AnnoncesGlobalesController.FilterDef filterDef)
	{
		return ConvertFilterDefToAnnoncesGlobalesSQL(filterDef, GetDefaultWhere(filterDef), GetImportParams());
	}

	private static string ConvertFilterDefToAnnoncesGlobalesSQL(AnnoncesGlobalesController.FilterDef filterDef, string defaultWhere, Dictionary<string, string> importParams)
	{
		string where = defaultWhere;
		for (int j = 0; j < filterDef.Fields.Length; j++)
		{
